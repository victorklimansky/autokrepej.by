...
function oxygen_activate() {
 
 		global $wpdb;
		
		add_option("theme_footer_text","This is a sample text, which you can replace in theme configuration panel");
		$footer_image = get_bloginfo('template_url')."/images/logo-oxygen-2.png";
		add_option("theme_footer_image",$footer_image);
		$theme_logo = get_bloginfo('template_url')."/images/logo-oxygen-1.png";
...
function orange_themes_info_message($content) {
 ...
 wp_register_theme_activation_hook('oxygen', 'oxygen_activate');
...
function theme_menu() {
	add_menu_page('Oxygen Management', 'Oxygen Management', 'administrator', 'theme-configuration', 'theme_configuration',get_template_directory_uri().'/images/control-panel-images/logo-orangethemes-1.png');
...
function theme_configuration() {
	...
	if($action == "page_settings") 
	{
		...
		//add theme logo
		$footer_logo_upload = $_POST["footer_logo_upload"];
		if($footer_logo_upload) {
			update_option("".$theme_name."_footer_image",$footer_logo_upload);
		}
		else delete_option("".$footer_logo_upload."_footer_image");
		
		//theme style settings
		$theme_color = $_POST["".$theme_name."_color"];
		$menu_color = $_POST["menu_color"];
		update_option("".$theme_name."_color",$theme_color);
		update_option("".$theme_name."_menu_color",$menu_color);
	...
	if($action == "gallery_settings") 
	{
		...
		$portfolio_items = $_POST["show_portfolio_items"];
		update_option("".$theme_name."_show_gallery_items",$gallery_items);
		update_option("".$theme_name."_show_portfolio_items",$portfolio_items);
	...
	if($action == "contact_settings") {
		...
		$contact_mail = $_POST["".$theme_name."_contact_mail"];
		update_option("".$theme_name."_contact_mail",$contact_mail);		
					...
					<div id="theme_general_settings">
						...
						<div class="settings">
							...
							<div class="tabs">
								...
								<a href="#" class="<?php if($_GET["pid"] == "theme_gallery_settings") { ?> active<?php } ?> config_stab" id="tab_theme_gallery_settings"><span>Gallery & Portfolio </span></a>
