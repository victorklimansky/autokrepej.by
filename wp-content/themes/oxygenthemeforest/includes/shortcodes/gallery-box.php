	...
	if(isset($atts['url'])) {
		...
		if(is_object($page)) {
			...
			if(is_numeric($id)) {
				...
				$content.=		'<div class="gallery-preview-box-title"><b>'.__('Photo gallery','oxygen').':</b><a href="'.$atts['url'].'">'.$page->post_title.'</a></div>';
				...
				$content.=		'<table><tr><td><a href="'.$atts['url'].'">'.__('<b>show<br />all<br />photos</b>','oxygen').'</a></td></tr></table>';
				$content.=		'</div>';
				$content.=	'</div>';
				$content.= '</div>';
